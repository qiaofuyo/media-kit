...
```powershell
ffmpeg -loglevel quiet -ss "${startTime}" -to "${endTime}" -fflags +genpts -i "${inputFile}" -c copy -movflags +faststart "${outputFile}"
ffmpeg -loglevel error -ss "${startTime}" -to "${endTime}" -i "${inputFile}" -c copy -fflags +genpts -movflags +faststart -y "${outputFile}"
```
